		...
		if msg.content_.ID == "MessageText" then
			...
			if is_naji(msg) then
				...
				elseif text:match("^(لیست) (.*)$") then
					...
					if matches == "مخاطبین" then
						...
						function (I, Naji)
							...
							tdcli_function ({
								...
								caption_ = "مخاطبین فتوبات شماره BOT-ID"}
							}, dl_cb, nil)
					...
					local list =  redis:smembers(a)
					local text = tostring(matches).." : \n"
					for i, v in pairs(list) do
						text = tostring(text) .. tostring(i) .. "-  " .. tostring(v).."\n"
					end
					writefile(tostring(apekt)..".txt", text)
					tdcli_function ({
						ID = "SendMessage",
...
ت					end
				...
				elseif text:match("^(پاسخگوی خودکار) (.*)$") then
					...
					if matches == "روشن" then
						...
						return send(msg.chat_id_, 0, "<i>پاسخگویی خودکار فتوبات فعال شد</i>")
					elseif matches == "خاموش" then
						redis:del("botBOT-IDautoanswer")
						return send(msg.chat_id_, 0, "<i>حالت پاسخگویی خودکار فتوبات غیر فعال شد.</i>")
					end
				elseif text:match("^(تازه سازی)$")then
					...
					return send(msg.chat_id_,msg.id_,"<i>تازه‌سازی آمار فتوبات شماره </i><code> BOT-ID </code> با موفقیت انجام شد.")
...
<b>]] .. tostring(links)..[[</b>
 😼 سازنده : @tlm20]]
				...
				elseif text:match("^(ترک کردن) (.*)$") then
					...
					send(msg.chat_id_, msg.id_, 'فتوبات از گروه مورد نظر خارج شد')
					tdcli_function ({
